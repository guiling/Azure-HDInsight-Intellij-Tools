...
import com.microsoft.azure.hdinsight.spark.common.SparkHelper;
...
public class AddHDInsightAdditionalClusterImpl {
    ...
    private static List<StorageAccount> getStorageAccountsFromResponseMessage(String responseMessage) throws StorageAccountResolveException {
        ...
        storageAccounts.add(new StorageAccount(defaultStorageName, storageKeysMap.get(defaultStorageName),false, null));

        for (String storageName : storageKeysMap.keySet()) {
            if (!storageName.equals(defaultStorageName)) {
                storageAccounts.add(new StorageAccount(storageName, storageKeysMap.get(storageName), false, null));
            }
        }
    ...
    public static List<StorageAccount> getStorageAccounts(String clusterEndpoint, String userName, String passwd)
        ...
        try {
            ...
            if (responseCode == 200) {
                message = SparkHelper.getResultFromHttpResponse(response).getMessage();

            } else if (responseCode == 401 || responseCode == 403) {
                throw new HDIException("Invalid Cluster Name or Cluster Url");
            } else {
            ...
            throw new HDIException("Wrong User Name or Password");
        } catch (StorageAccountResolveException e2) {
