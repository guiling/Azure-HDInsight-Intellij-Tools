...
public class BlobExplorerFileEditor implements FileEditor {
    ...
    private LinkedList<BlobDirectory> directoryQueue = new LinkedList<>();
            ...
            public void run(@NotNull ProgressIndicator progressIndicator) {
                try {
                    ...
                    if (directoryQueue.peekLast() == null) {
                        directoryQueue.addLast(StorageClientImpl.getInstance().getRootDirectory(storageAccount, blobContainer));
                    }

                    blobItems = StorageClientImpl.getInstance().getBlobItems(storageAccount, directoryQueue.peekLast());
                    ...
                    DefaultLoader.getUIHelper().showException("An error occurred while attempting to query blob list.", ex,
                            "HDInsight Services Explorer - Error Querying Blobs", false, true);
                }
            }
        });
    }
            ...
            if (JOptionPane.showConfirmDialog(mainPanel, "Are you sure you want to delete this blob?", "Delete Blob", JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE) == JOptionPane.OK_OPTION) {
                ...
                ProgressManager.getInstance().run(new Task.Backgroundable(project, "Deleting blob...", false) {
                    ...
                    public void run(@NotNull ProgressIndicator progressIndicator) {
                        progressIndicator.setIndeterminate(true);
                        try {
                            StorageClientImpl.getInstance().deleteBlobFile(storageAccount, blobItem);
                            ...
                            DefaultLoader.getUIHelper().showException("An error occurred while attempting to delete blob.", ex,
                                    "HDInsight Services Explorer - Error Deleting Blob", false, true);
                        }
                    }
                });
            }
                    ...
                    try {
                        ...
                        try {
                            Future<?> future = ApplicationManager.getApplication().executeOnPooledThread(new Runnable() {
                                ...
                                public void run() {
                                    try {
                                        StorageClientImpl.getInstance().downloadBlobFileContent(storageAccount, fileSelection, bufferedOutputStream);
                        ...
                        DefaultLoader.getUIHelper().showException("An error occurred while attempting to download Blob.", e,
                                "HDInsight Services Explorer - Error Downloading Blob", false, true);
                    }
                }
            });
        }
                ...
                try {
                    ...
                    try {
                        ...
                        Future<Void> future = ApplicationManager.getApplication().executeOnPooledThread(new Callable<Void>() {
                            @Override
                            public Void call() throws HDIException {
                                try {
                        ...
                        while (!future.isDone()) {
                            ...
                            if (progressIndicator.isCanceled()) {
                                ...
                                for (BlobItem blobItem : StorageClientImpl.getInstance().getBlobItems(storageAccount, blobDirectory)) {
                                    if (blobItem instanceof BlobFile && blobItem.getPath().equals(path)) {
                                        StorageClientImpl.getInstance().deleteBlobFile(storageAccount, (BlobFile) blobItem);
                                    }
                                }
                            }
                        }
                        ...
                        try {
                            ...
                            for (String pathDir : path.split("/")) {
                                for (BlobItem blobItem : StorageClientImpl.getInstance().getBlobItems(storageAccount, directoryQueue.getLast())) {
                            ...
                            DefaultLoader.getUIHelper().showException("An error occurred while attempting to show new blob", e,
                                    "HDInsight Services Explorer - Error Showing New Blob", false, true);
                        }
                    ...
                    DefaultLoader.getUIHelper().showException("An error occurred while attempting to upload Blob.", e,
                            "HDInsight Services Explorer - Error Uploading Blob", false, true);
                }
            }
        });
    }
